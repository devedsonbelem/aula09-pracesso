...
public class UsersControl {
	...
	// Aqui no Sistema é o primeiro contato com estranho no meu site, aqui
	// sim deve ser muito seguro ...
	@PostMapping("/create")
	...
	public ResponseEntity<?> login(@RequestBody UsersRequest request,
		...
		try {
			...
			users.setName(request.getName());
			users.setPassword(service.criptografia(request.getPassword()));
			Users resp = usersRespository.findByNameAndPassword(users.getName(), users.getPassword());
